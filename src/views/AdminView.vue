<template>
  <div class="min-h-screen bg-slate-50">
      ...
      <div class="w-full max-w-sm">
        <div class="text-center mb-8">
          ...
          <p class="text-gray-500 text-sm mt-1">
            SkillSprint workshop registrations
          </p>
        </div>
        ...
        >
          <div class="flex flex-col gap-1.5">
            <label for="password" class="text-gray-700 text-sm font-semibold"
              >Password</label
            >
            <input
              id="password"
              ...
              autofocus
              class="border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus-visible:outline-2 focus-visible:outline-blue-600 transition-colors duration-200"
            />
          </div>
          ...
          <button
            type="submit"
            :disabled="loggingIn"
            class="bg-blue-600 text-white py-3 rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors duration-200 disabled:opacity-50 cursor-pointer"
          >
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-0.5">
            Workshop signups and email delivery
          </p>
        </div>
        <div class="flex items-center gap-2">
          <button
            @click="refreshAll"
            :disabled="loading"
            class="border border-gray-200 bg-white text-gray-700 px-4 py-2 rounded-lg text-sm font-semibold hover:border-gray-300 transition-colors duration-200 disabled:opacity-50 cursor-pointer"
          >
            {{ loading ? "Refreshing…" : "Refresh" }}
          </button>
          <button
            @click="logout"
            class="border border-gray-200 bg-white text-gray-500 px-4 py-2 rounded-lg text-sm font-semibold hover:border-gray-300 hover:text-gray-700 transition-colors duration-200 cursor-pointer"
          >
            Sign out
          </button>
        </div>
      ...
      <!-- Tabs -->
      <div
        class="flex gap-2 overflow-x-auto mb-6 border-b border-gray-200 -mx-4 px-4 sm:mx-0 sm:px-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        <button
          v-for="t in tabs"
          :key="t.id"
          @click="tab = t.id"
          class="shrink-0 whitespace-nowrap px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors duration-200 cursor-pointer"
          :class="
            tab === t.id
      ...
      <div v-show="tab === 'people'">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          ...
          >
            <div
              ...
              :class="card.labelClass || 'text-gray-500'"
            >
              {{ card.label }}
            </div>
            <div
              class="text-2xl font-extrabold tabular-nums"
              :class="card.valueClass || 'text-gray-900'"
            >
              {{ card.value }}
            </div>
          </div>
        ...
        >
          <p class="text-amber-900 font-bold text-sm mb-1">
            {{ stuckWarning.title }}
          </p>
        ...
        <div
          v-if="stats"
          class="bg-white border border-gray-200 rounded-xl px-5 py-4 mb-6"
        >
          <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-6">
              ...
              >
                <span
                  class="text-xs font-bold uppercase tracking-wider text-gray-700 w-14"
                  >{{ name }}</span
                >
                <div
                  class="w-28 h-1.5 bg-gray-100 rounded-full overflow-hidden"
                >
                  <div
                ...
                <span class="text-xs font-mono tabular-nums text-gray-500"
                  >{{ q.used }}/{{ q.limit }}</span
                >
                <span
                  v-if="!q.configured"
          ...
          <div v-if="senderSummary" class="mt-3 pt-3 border-t border-gray-100">
            ...
            <span
              ...
              :class="
                senderWarning ? 'text-amber-700 font-bold' : 'text-gray-700'
              "
            >
              {{ senderSummary }}
            </span>
        ...
        <div class="flex flex-wrap items-center gap-3 mb-4">
          ...
          <select
            ...
            class="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white cursor-pointer focus-visible:outline-2 focus-visible:outline-blue-600 max-w-full"
          >
            <option :value="WELCOME_TEMPLATE">
              Registration email (welcome + schedule)
            </option>
            <option
          ...
          <span
            v-if="emailView !== WELCOME_TEMPLATE && !workshops.length"
            class="text-xs text-gray-400"
            >No workshops yet</span
          >
          <span
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4">
          <input
            ...
            class="flex-1 min-w-[200px] border border-gray-200 rounded-lg px-3.5 py-2 text-sm bg-white focus-visible:outline-2 focus-visible:outline-blue-600 transition-colors duration-200"
          />

          <select
            v-model="statusFilter"
            @change="loadRegistrations"
            class="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white cursor-pointer focus-visible:outline-2 focus-visible:outline-blue-600"
          >
          ...
          <button
            ...
            class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors duration-200 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          >
            {{
              sendingAll
                ? "Sending…"
          ...
          <button
            ...
            @click="retryFailed"
            :disabled="sendingAll"
            class="border border-amber-300 bg-amber-50 text-amber-800 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-amber-100 transition-colors duration-200 disabled:opacity-40 cursor-pointer"
          ...
          <button
            ...
            class="border border-gray-200 bg-white text-gray-700 px-4 py-2 rounded-lg text-sm font-semibold hover:border-gray-300 transition-colors duration-200 cursor-pointer"
          >
            Download CSV
          </button>
          <button
            @click="downloadCsv('all')"
            class="border border-gray-200 bg-white text-gray-500 px-3 py-2 rounded-lg text-xs font-semibold hover:border-gray-300 transition-colors duration-200 cursor-pointer"
          >
            All
          </button>
        </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    class="px-4 py-3 text-gray-400 text-xs whitespace-nowrap tabular-nums"
                  >
                    {{ shortDate(r.created_at) }}
                  </td>
                  <td class="px-4 py-3 whitespace-nowrap">
                    <span
                      class="inline-block px-2 py-0.5 rounded text-[11px] font-bold uppercase tracking-wide"
                      :class="
                        r.is_stuck
                          ? 'bg-amber-100 text-amber-800'
                          : statusClass(r.email_status)
                      "
                    >
                    ...
                    <span
                      v-if="r.provider"
                      class="ml-1.5 text-[10px] text-gray-400 font-mono"
                      >{{ r.provider }}</span
                    >
                    <div
                      v-if="r.last_error"
                      class="text-[11px] text-red-500 mt-1 max-w-xs truncate"
                      :title="r.last_error"
                    >
                      {{ r.last_error }}
                    </div>
                  ...
                  <td class="px-4 py-3 whitespace-nowrap">
                    <button
                      ...
                      :disabled="
                        sendingId === r.id ||
                        (r.email_status === 'processing' && !r.is_stuck)
                      "
                      class="text-blue-600 text-xs font-bold hover:text-blue-800 transition-colors duration-200 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                      :title="
                        r.email_status === 'processing' && !r.is_stuck
          ...
          >
            ...
            <div class="flex gap-2">
              <button
                @click="changePage(-1)"
                :disabled="page === 1"
                class="px-3 py-1 rounded border border-gray-200 bg-white text-xs font-semibold disabled:opacity-40 cursor-pointer"
              >
                Prev
              </button>
              <button
      ...
      <div v-show="tab === 'workshops'">
        <div class="bg-white border border-gray-200 rounded-xl p-5 mb-6">
          <h2 class="font-extrabold text-gray-900 mb-4">
            {{ editing ? "Edit workshop" : "Add a workshop" }}
          </h2>
          <form
            @submit.prevent="saveWorkshop"
          ...
          >
            <div class="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <input
                ...
                required
                :class="adminInput"
                placeholder="Git & GitHub for Beginners"
              />
            </div>
            <div class="flex flex-col gap-1.5 sm:col-span-2">
              <label
              ...
              <textarea
                ...
                rows="2"
                :class="adminInput"
                placeholder="What people will actually walk away able to do."
              ></textarea>
            </div>
            <div class="flex flex-col gap-1.5">
              <label
              ...
              >
                Starts at *
                <span class="text-blue-600">({{ SITE_TIME_ZONE_LABEL }})</span>
              </label>
              <input
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <span class="text-[11px] text-gray-400"
                >Shown on the card and in the email.</span
              >
            </div>
            <div class="flex flex-col gap-1.5">
              <label
            ...
            <div class="flex flex-col gap-1.5 sm:col-span-2">
              ...
              />
              <span class="text-[11px] text-gray-400">
                Never shown on the public page. Only emailed to registered
                people when you press
            ...
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              <span class="text-sm text-gray-700 font-medium"
                >Published (visible on the site)</span
              >
            </label>
            ...
            <div class="sm:col-span-2 flex items-center gap-2">
              <button
                ...
                class="bg-blue-600 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors duration-200 disabled:opacity-50 cursor-pointer"
              >
                {{
                  savingWorkshop
                    ? "Saving…"
        ...
        >
          ...
          <p class="text-gray-600 text-xs mb-3">
            Goes to one address only. Nobody registered receives anything, and
            no delivery status is recorded — but it does count against the daily
            provider quota.
          </p>
          <form
          ...
          >
            <input
              ...
              class="flex-1 min-w-[220px] border border-gray-200 rounded-lg px-3.5 py-2 text-sm bg-white focus-visible:outline-2 focus-visible:outline-blue-600"
            />
            <button
              type="submit"
              :disabled="sendingTest"
              class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors duration-200 disabled:opacity-50 cursor-pointer"
            >
              ...
              <tbody>
                <tr v-if="!workshops.length">
                  <td
                    ...
                    class="px-4 py-10 text-center text-gray-400 text-sm"
                  >
                    No workshops yet. Add one above and it appears on /workshops
                    immediately.
                  </td>
                </tr>
                <tr
                ...
                >
                  <td
                    class="px-4 py-3 text-gray-500 text-xs whitespace-nowrap tabular-nums"
                  >
                  ...
                  <td class="px-4 py-3 whitespace-nowrap">
                    <select
                      ...
                      class="px-2 py-1 rounded text-[11px] font-bold uppercase tracking-wide border-0 cursor-pointer disabled:opacity-50"
                      :class="statusClass(w.status)"
                    >
                      <option
                        v-for="opt in WORKSHOP_STATUSES"
                        :key="opt"
                        :value="opt"
                      >
                        {{ opt }}
                  ...
                  <td class="px-4 py-3 whitespace-nowrap">
                    <button
                      ...
                      :title="
                        w.is_published
                          ? 'Visible on /workshops — click to unpublish'
                          : 'Hidden from /workshops — click to publish'
                      "
                    >
                  ...
                  <td class="px-4 py-3 whitespace-nowrap">
                    ...
                    <template v-else>
                      ...
                      <div
                        ...
                        :class="
                          needsLink(w) ? 'text-amber-600' : 'text-gray-400'
                        "
                      >
                        not emailed yet
                      </div>
                  ...
                  <td class="px-4 py-3 whitespace-nowrap">
                    <button
                      ...
                      class="text-emerald-600 text-xs font-bold hover:text-emerald-800 mr-3 disabled:text-gray-300 disabled:cursor-not-allowed cursor-pointer"
                      :title="
                        w.meeting_link
                    ...
                    <button
                      ...
                      :disabled="sendingLinkId === w.id || !w.meeting_link"
                      class="text-amber-600 text-xs font-bold hover:text-amber-800 mr-3 disabled:text-gray-300 disabled:cursor-not-allowed cursor-pointer"
                    ...
                    <button
                      ...
                      class="text-blue-600 text-xs font-bold hover:text-blue-800 mr-3 disabled:text-gray-300 disabled:cursor-not-allowed cursor-pointer"
                    ...
                    <button
                      ...
                      class="text-blue-600 text-xs font-bold hover:text-blue-800 mr-3 cursor-pointer"
                    >
                      Edit
                    </button>
                    <button
                      @click="deleteWorkshop(w)"
                      class="text-red-500 text-xs font-bold hover:text-red-700 cursor-pointer"
                    >
                      Delete
                    </button>
                  </td>
      ...
      <div v-show="tab === 'team'">
        <div class="bg-white border border-gray-200 rounded-xl p-5 mb-6">
          ...
          <p class="text-gray-500 text-sm mb-4">
            They appear on the public /team page straight away.
          </p>
          ...
          <form @submit.prevent="saveMember" class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <textarea
                ...
                required
                :class="adminInput"
                placeholder="What they actually do here."
              ></textarea>
            </div>
            ...
            <div class="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <div class="flex items-center gap-4">
                <div
                  class="w-16 h-16 rounded-full bg-gray-100 border border-gray-200 overflow-hidden flex-shrink-0 flex items-center justify-center"
                >
                  <img
                    v-if="tForm.image"
                    :src="tForm.image"
                  ...
                  />
                  <span v-else class="text-gray-300 text-xl" aria-hidden="true"
                    >👤</span
                  >
                </div>
                <div class="flex flex-col gap-1">
                  <input
                    ...
                    accept="image/*"
                    @change="pickImage"
                    class="text-xs text-gray-600 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border file:border-gray-200 file:bg-white file:text-xs file:font-semibold file:text-gray-700 file:cursor-pointer cursor-pointer"
                  />
                  <span class="text-[11px] text-gray-400"
                    >JPG or PNG, under 2MB. Resized to 400px before
                    saving.</span
                  >
                  <button
                    v-if="tForm.image"
            ...
            <div class="sm:col-span-2 flex items-center gap-3">
              ...
              <span v-if="memberError" class="text-red-600 text-sm">{{
                memberError
              }}</span>
            </div>
          </form>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                  <th
                    v-for="h in ['', 'Name', 'Role', 'Bio', 'Actions']"
                    :key="h"
                    class="text-left px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider text-gray-500 whitespace-nowrap"
                  >
                    {{ h }}
              ...
              <tbody>
                ...
                >
                  <td class="px-4 py-3">
                    <div
                      class="w-9 h-9 rounded-full bg-gray-100 overflow-hidden flex items-center justify-center"
                    >
                      <img
                        v-if="m.image"
                        :src="m.image"
                  ...
                  <td
                    class="px-4 py-3 text-gray-400 text-xs max-w-md truncate"
                    :title="m.bio"
                  >
                    {{ m.bio }}
                  </td>
    ...
    >
      ...
      >
        ...
        <p v-if="t.body" class="text-xs leading-relaxed opacity-90">
          {{ t.body }}
        </p>
      </div>
    </div>
...
const summaryCards = computed(() => {
  ...
  return [
    ...
    {
      label: "Delivered",
      value: s.email.delivered,
      valueClass: "text-emerald-600",
    },
    {
      label: "Not received",
      value: s.email.notReceived,
      accent: s.email.notReceived
...
const stuckWarning = computed(() => {
  ...
  return {
    ...
    body:
      ...
      (email.oldest_lock
        ? ` Oldest has been held ${lockAge(email.oldest_lock)}.`
        : ""),
  };
});
  ...
  for (const name of ["resend", "brevo"]) {
    ...
    if (info?.placeholder) {
      return {
        ...
        body:
          `${name} is sending from @${info.domain}, which is the placeholder in ` +
    ...
    if (!info?.set) {
      return {
        ...
        body:
          `${name} has an API key but no From address, so every send fails. ` +
...
const loadStats = async () => {
  try {
    ...
    /* the refresh button surfaces this */
  }
};
...
const reportSendResult = (data, successTitle) => {
  ...
  if (data.code === "QUOTA_EXHAUSTED") {
    ...
    toast(
      "warn",
      "Daily limit reached",
      `${detail ? detail + ". " : ""}Still queued — it will send automatically after the limit resets ${resetCountdown.value}.`,
      9000,
    );
    return;
  }
  if (data.code === "NO_PROVIDER") {
    toast(
      ...
      "Add RESEND_API_KEY or BREVO_API_KEY. The email stays queued until then.",
      9000,
    );
    return;
  }
  ...
  try {
    ...
    if (!data.ok) {
      toast(
        "error",
        "Could not update",
        data.message || "The change was not saved.",
      );
      return null;
    }
...
const changeStatus = async (w, status) => {
  ...
  const updated = await patchWorkshop(
    w,
    { status },
    `Status set to ${status}`,
    w.is_published
      ? WHERE_IT_SHOWS[status]
      : "Saved — still a draft, so not on the site yet.",
  );
  ...
  try {
    ...
    if (data.ok && data.test) {
      toast(
        ...
        `${data.provider ? `Via ${data.provider}. ` : ""}Subject: ${data.subject}`,
        9000,
      );
  ...
  try {
    ...
    for (let pass = 0; pass < MAX_PASSES; pass++) {
      ...
      if (res.status === 504 || res.status === 502) {
        toast(
          "warn",
          "That pass timed out",
          `${totalSent} sent so far. The rest stay queued — press the button again to continue.`,
          9000,
        );
        return;
      }
    ...
    if (data.code === "NO_LINK") {
      toast(
        "warn",
        "No meeting link set",
        "Add one to this workshop first, then send.",
      );
    } else if (data.code === "QUOTA_EXHAUSTED" || data.code === "NO_PROVIDER") {
      reportSendResult(data, "");
    } else if (data.ok) {
      ...
      toast(
        "success",
        `Meeting link sent — ${s.sent ?? 0} email${s.sent === 1 ? "" : "s"}`,
...
const saveWorkshop = async () => {
  ...
  try {
    ...
    const url = editing.value
      ? `/api/workshops?id=${editing.value}`
      : "/api/workshops";
    ...
    if (data.ok) {
      ...
      toast(
        "success",
        editing.value ? "Workshop updated" : "Workshop added",
        wForm.is_published
          ? "It's on /workshops now."
          : "Saved as a draft, not on the site yet.",
      );
      if (saved.meeting_link && !saved.link_sent_at) {
        toast(
          "warn",
          "Join link not emailed yet",
          "The link is saved but nobody has received it. Press Send link when you are ready.",
          9000,
        );
      }
      resetWorkshopForm();
      ...
      toast(
        "error",
        "Could not save",
        data.message || "Check the fields and try again.",
      );
    }
  } catch {
  ...
  try {
    ...
    if (data.ok) {
      toast(
        "success",
        "Team member added",
        `${tForm.name.trim()} is now on /team.`,
      );
      ...
      memberError.value =
        data.message || "Could not save. Check the fields and try again.";
    }
  } catch {
...
const deleteMember = async (m) => {
  if (
    !window.confirm(
      `Remove ${m.name} from the team page? This cannot be undone.`,
    )
  )
    return;
  try {
    const res = await fetch(`/api/team-members?id=${m.id}`, {
      method: "DELETE",
    });
    const data = await res.json().catch(() => ({}));
    if (data.ok) {
