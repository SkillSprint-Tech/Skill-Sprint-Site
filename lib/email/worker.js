...
async function buildRegistrationMessage(pool, job, schedule, sessionNumbers) {
  ...
  // Reminder jobs carry their workshop in the template key: `reminder:<workshop_id>` or `reminder_24h:<id>` or `reminder_1h:<id>`.
  ...
  if (job.template.startsWith('reminder:') || job.template.startsWith('reminder_24h:') || job.template.startsWith('reminder_1h:')) {
    const workshopId = job.template.replace(/^reminder(_24h|_1h)?:/, '')
