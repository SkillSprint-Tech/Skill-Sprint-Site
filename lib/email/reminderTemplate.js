...
${
  ...
  speakerLine
    ...
    <tr><td style="padding-top:14px;text-align:center;">
      <a href="${escapeHtml(siteUrl)}/portal?ticket=${escapeHtml(registration.id || '')}"
         style="display:inline-block;color:${BRAND};font-size:13px;font-weight:700;text-decoration:none;">
        🎟️ View Digital Check-in QR Pass & Portal &rarr;
      </a>
    </td></tr>

    <tr><td style="padding-top:12px;">
      <p style="color:${MUTED};font-size:13px;line-height:1.6;margin:0;font-style:italic;">
        Please try to join 10 minutes early so we can admit everyone and start right on time.
      </p>
    </td></tr>
  ...
  const text = [
    ...
    `Digital Check-in Pass: ${siteUrl}/portal?ticket=${registration.id || ''}`,
    '',
    'Note: Please try to join 10 minutes early so we can admit everyone and start right on time.',
    '',
    'Stay Connected',
...
export const renderReminder = renderWorkshopReminder
