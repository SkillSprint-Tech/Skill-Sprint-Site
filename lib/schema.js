...
const STATEMENTS = [
  ...
  `ALTER TABLE registrations ADD COLUMN IF NOT EXISTS tags TEXT[] NOT NULL DEFAULT '{}'`,

  // Distinct check-in records per workshop
  `CREATE TABLE IF NOT EXISTS workshop_checkins (
     ...
     created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
     CONSTRAINT workshop_checkins_unique UNIQUE (registration_id, workshop_id)
   )`,
  `CREATE INDEX IF NOT EXISTS workshop_checkins_reg_idx ON workshop_checkins (registration_id)`,
  `CREATE INDEX IF NOT EXISTS workshop_checkins_workshop_idx ON workshop_checkins (workshop_id)`,
...
export async function ensureSchema() {
  ...
  for (const statement of STATEMENTS) {
    try {
      ...
      console.warn('ensureSchema warning for statement:', statement.trim().slice(0, 80), '->', err.message)
    }
  }
