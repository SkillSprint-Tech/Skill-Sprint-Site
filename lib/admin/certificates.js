...
const OPS = {
  ...
  async 'populate-from-checkins'(body, res) {
    const workshopId = requireId(body.workshopId)
    await assertWorkshop(workshopId)
    const { rows } = await getPool().query(
      `SELECT r.full_name AS "fullName", r.email
         FROM workshop_checkins wc
         JOIN registrations r ON r.id = wc.registration_id
        WHERE wc.workshop_id = $1`,
      [workshopId]
    )
    if (!rows.length) {
      return ok(res, {
        ...
        totalCheckedIn: 0,
        alreadyPresent: 0,
        message: 'No checked-in attendees found for this workshop.',
      })
    }
    const inserted = await insertRecipients(workshopId, rows, 'attendance')
    return ok(res, {
      inserted,
      totalCheckedIn: rows.length,
      alreadyPresent: rows.length - inserted,
      message: `Successfully populated ${inserted} attendee${inserted === 1 ? '' : 's'} from check-in list.`,
    })
  },
