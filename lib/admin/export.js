...
import { buildQuery, WELCOME_TEMPLATE } from './registrations.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // The template is NOT reset by scope=all: "export everyone" still means everyone for
    // the email currently on screen, otherwise the download silently changes subject.
    const template = getQueryParam(req, 'template') || WELCOME_TEMPLATE

    const { sql, params } = buildQuery({ search, status, template })
    const { rows } = await pool.query(sql, params)

    // Name the file after the email it describes, so a folder of exports stays legible.
    const label = template === WELCOME_TEMPLATE
      ? 'registrations'
      : `meeting-link-${template.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}`
    const filename = `skillsprint-${label}-${new Date().toISOString().slice(0, 10)}.csv`
    ...
    const isWorkshopScope = template.startsWith('reminder:')
    const targetWorkshopId = isWorkshopScope ? template.replace('reminder:', '') : null

    const exportColumns = [...COLUMNS]
    if (isWorkshopScope) {
      exportColumns.push(['workshop_attended', 'Attended This Workshop'])
    } else {
      exportColumns.push(['attended_count', 'Workshops Attended'])
    }

    const lines = [exportColumns.map(([, label]) => csvCell(label)).join(',')]
    for (const row of rows) {
      const checkins = Array.isArray(row.checkins) ? row.checkins : []
      const extendedRow = {
        ...row,
        attended_count: checkins.length,
        workshop_attended: checkins.some((c) => c.workshop_id === targetWorkshopId) ? 'Yes' : 'No',
      }
      lines.push(exportColumns.map(([key]) => csvCell(extendedRow[key])).join(','))
    }
