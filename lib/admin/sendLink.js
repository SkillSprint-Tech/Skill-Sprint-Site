...
export default async function handler(req, res) {
  ...
  try {
    ...
    // How many of THIS workshop's links are still waiting. The worker deliberately stops
    // before the function's time limit, so a large list needs more than one pass — the
    // panel uses this to keep going rather than leaving people unsent.
    const { rows: left } = await pool.query(
      `SELECT count(*)::int AS n
         FROM email_jobs
        WHERE template = $1 AND status IN ('pending', 'deferred', 'processing')`,
      [template]
    )
