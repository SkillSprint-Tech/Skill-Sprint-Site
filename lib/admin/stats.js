...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [totals, statuses, usage, recent, stuckRows, trendRows, uniRows] = await Promise.all([
      pool.query(`
        SELECT
          ...
          (SELECT count(DISTINCT registration_id)::int FROM workshop_checkins) AS attendees_checked_in,
          (SELECT count(*)::int FROM workshop_checkins) AS total_checkins
      `),
      pool.query(
      ...
      `),
      pool.query(`
        SELECT COALESCE(NULLIF(TRIM(university), ''), 'Unspecified') AS name,
               count(*)::int AS count
          FROM registrations
         GROUP BY 1
         ORDER BY count DESC
    ...
    return ok(res, {
      totals: {
        ...
        attendees_checked_in: totals.rows[0].attendees_checked_in || 0,
        total_checkins: totals.rows[0].total_checkins || 0,
      },
      template,
      email: {
      ...
      trend: trendRows.rows.map((r) => ({ day: r.day, count: Number(r.count) })),
      topUniversities: uniRows.rows.map((r) => ({ name: r.name, count: Number(r.count) })),
    })
  } catch (error) {
