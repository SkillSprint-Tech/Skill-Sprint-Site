...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!expected) {
      ...
      return fail(res, 'NOT_CONFIGURED', 'Admin access is not configured on this server: ADMIN_PASSWORD is not set in environment.', 500)
    }
    ...
    return fail(res, 'SERVER_ERROR', error.message || 'Could not complete sign in.', 500)
  }
}
