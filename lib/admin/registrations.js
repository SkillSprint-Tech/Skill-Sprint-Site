...
export function buildQuery({ search, status, limit, offset, template = WELCOME_TEMPLATE }) {
  ...
  let sql = `
    SELECT
      ...
      r.skill_level, r.interests, r.created_at, r.attended, r.checked_in_at,
      r.admin_notes, r.tags,
      COALESCE(
        (SELECT json_agg(json_build_object('workshop_id', wc.workshop_id, 'checked_in_at', wc.checked_in_at))
           FROM workshop_checkins wc
          WHERE wc.registration_id = r.id),
        '[]'::json
      ) AS checkins,
      j.id AS job_id,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      const { id, admin_notes, tags } = req.body || {}
      if (!id) return fail(res, 'MISSING_ID', 'Attendee id is required.', 400)
      const cleanNotes = typeof admin_notes === 'string' ? admin_notes.trim() : undefined
      const cleanTags = Array.isArray(tags) ? tags.map((t) => String(t).trim()).filter(Boolean) : undefined
      ...
      if (!updates.length) {
        return fail(res, 'NO_CHANGES', 'No fields to update.', 400)
      }

      const { rows } = await pool.query(
        `UPDATE registrations SET ${updates.join(', ')} WHERE id = $1 RETURNING id, admin_notes, tags`,
        params
      )
      if (!rows.length) return fail(res, 'NOT_FOUND', 'Attendee not found.', 404)
      return ok(res, { attendee: rows[0], message: 'Updated attendee notes and tags.' })
    }
