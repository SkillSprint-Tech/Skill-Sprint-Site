...
export async function insertRecipients(workshopId, rows, source) {
  ...
  const { rowCount } = await query(
    `INSERT INTO certificate_recipients (workshop_id, full_name, email, source, verification_code)
     SELECT $1, t.full_name, t.email, $4, lower(substring(replace(gen_random_uuid()::text, '-', ''), 1, 16))
