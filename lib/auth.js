...
import { parseCookies, clientIp, fail } from './http.js'
...
const PORTAL_TTL_MS = 30 * 24 * 60 * 60 * 1000 // 30 days

export function createPortalToken(email) {
  const body = base64url(JSON.stringify({ email: String(email).toLowerCase(), iat: Date.now(), exp: Date.now() + PORTAL_TTL_MS }))
  return `${body}.${sign(body)}`
}
...
export function verifyPortalToken(token) {
  ...
  try {
    const claims = JSON.parse(Buffer.from(body, 'base64url').toString('utf8'))
    if (typeof claims.exp === 'number' && claims.exp > Date.now() && claims.email) {
      return claims.email
    }
    return null
  } catch {
...
export function portalCookie(token) {
  const parts = [
    `${PORTAL_COOKIE}=${token}`,
    'HttpOnly',
    'Path=/',
...
export function clearedPortalCookie() {
  return `${PORTAL_COOKIE}=; HttpOnly; Path=/; SameSite=Lax; Max-Age=0`
}

export function getPortalUser(req) {
  const cookie = parseCookies(req)[PORTAL_COOKIE]
  if (cookie) {
    const email = verifyPortalToken(cookie)
